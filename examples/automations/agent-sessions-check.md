...
automation:
  ...
  steps:
    - id: load-guidance
      prompt: |-
        Load and follow guidance/ATTENTION.md.
    - id: check-sessions
      prompt: |-
        Check which of my long-running agent or terminal sessions need my attention,
    ...
    - id: apply-decline-rule
      prompt: |-
        **For each session that looks like it needs attention, apply the decline rule
    ...
    - id: reconcile-and-report
      prompt: |-
        Before reporting, check your durable record for anything still open whose
    ...
    - id: read-only-guard
      prompt: |-
        This is a read-only run with respect to the sessions themselves: do NOT send
...
Watches your long-running agent and terminal sessions and surfaces only the ones genuinely waiting on a decision from you. The steps live in the frontmatter; this body is a human-facing description only.
