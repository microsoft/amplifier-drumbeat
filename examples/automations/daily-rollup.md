...
automation:
  ...
  steps:
    - id: confirm-fire-time
      prompt: |-
        Report the current wall-clock time and confirm whether this run fired at or
    ...
    - id: gather-rollup
      prompt: |-
        Gather and state the rollup, covering the last 24 hours, as your reply to
        THIS step:
    ...
    - id: emit-rollup
      prompt: |-
        End this step's reply with the **complete rollup**: restate the sections from
...
A once-a-day status report that summarizes what closed, what is still open, and what is blocked. It reports only -- it never acts. The steps live in the frontmatter.
